...
This script provides a flexible PostgreSQL data migration tool that supports:
1. MySQL configuration via config file or command line arguments
1. PostgreSQL configuration via config file or command line arguments
...
MIGRATION_DB_VERSION_MARKER = "postgres_migration.database.version"
...
class MigrationDatabase:
    ...
    def __init__(self, config: MigrationConfig):
        self.config = config
        self.db = PostgresqlDatabase(config.database, host=config.host, port=config.port, user=config.user, password=config.password)
        self.migrator = PostgresqlMigrator(self.db)
    ...
    def table_exists(self, table_name: str) -> bool:
        cursor = self.execute_sql("SELECT COUNT(*) FROM information_schema.tables WHERE table_catalog = CURRENT_DATABASE() AND table_name = %s", (table_name,))
        return cursor.fetchone()[0] > 0

    def column_exists(self, table_name: str, column_name: str) -> bool:
        cursor = self.execute_sql("SELECT COUNT(*) FROM information_schema.columns WHERE table_catalog = CURRENT_DATABASE() AND table_name = %s AND column_name = %s", (table_name, column_name))
        return cursor.fetchone()[0] > 0
    ...
    def get_column_type(self, table_name: str, column_name: str) -> str | None:
        ...
        cursor = self.execute_sql("SELECT data_type FROM information_schema.columns WHERE table_catalog = CURRENT_DATABASE() AND table_name = %s AND column_name = %s", (table_name, column_name))
        row = cursor.fetchone()
    ...
    def upsert_system_setting(self, name: str, value: str, source: str = "migration", data_type: str = "string"):
        ...
        self.execute_sql(
            ...
            ON CONFLICT ("name") DO UPDATE SET "source" = VALUES("source"),
    ...
    def execute(self) -> tuple[int, list]:
        ...
        for i in range(0, len(records), batch_size):
            ...
            for tenant_id, llm_factory in batch:
                ...
                placeholders.append("(%s, %s, %s, %s, to_timestamp(%s), %s, to_timestamp(%s))")
                params.extend(
                    [
                        record_id,
                        llm_factory,
        ...
        for i in range(0, len(records), batch_size):
            ...
            for tenant_id, llm_factory, api_key, status, provider_id in batch:
                ...
                values.append(
                    ...
                    f"{current_ts * 1000}, to_timestamp({current_ts}), "
                    f"{current_ts * 1000}, to_timestamp({current_ts}))"
                )
